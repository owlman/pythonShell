#!/usr/bin/env python3
"""
单元测试脚本：test_create_book_project.py
测试 Create_book_project.py 模块
"""
...
class TestCreateBookProject(unittest.TestCase):
    ...
    @patch('sys.argv', ['create_book_project.py', '/tmp/projects'])
    ...
    def test_main_success_default_name(self, mock_zipfile, mock_rmtree, mock_makedirs,
        ...
        """测试成功创建项目（使用默认项目名）"""
        # 模拟路径存在性
        mock_exists.side_effect = lambda path: False if 'book_proj' in path else True
    ...
    @patch('sys.argv', ['create_book_project.py', '/tmp/projects', 'my_book'])
    @patch('common.print_banner')
    @patch('os.path.exists')
    @patch('os.makedirs')
    @patch('shutil.rmtree')
    ...
    def test_main_success_custom_name(self, mock_zipfile, mock_rmtree, mock_makedirs,
                                     mock_exists, mock_print_banner):
        """测试成功创建项目（使用自定义项目名）"""
        mock_exists.side_effect = lambda path: False if 'my_book' in path else True
        mock_zip = MagicMock()
        mock_zipfile.return_value.__enter__.return_value = mock_zip
        ...
        with patch('sys.exit') as mock_exit:
            ...
            # 验证目标目录路径包含自定义名称
            expected_target = '/tmp/projects/my_book'
            mock_exists.assert_any_call(expected_target)
    ...
    @patch('sys.argv', ['create_book_project.py'])
    ...
    @patch('sys.argv', ['create_book_project.py', 'dir1', 'name1', 'extra'])
    @patch('sys.stdout.write')
    ...
    def test_main_existing_target_directory(self, mock_zipfile, mock_rmtree, mock_makedirs,
        ...
        """测试目标目录已存在（应被删除）"""
        # 模拟目标目录已存在
        mock_exists.side_effect = lambda path: True if 'book_proj' in path else False
    ...
    def test_main_template_not_found(self, mock_rmtree, mock_makedirs, mock_exists,
        ...
        with patch('os.path.dirname') as mock_dirname:
            with patch('os.path.abspath') as mock_abspath:
                mock_abspath.return_value = '/path/to/src/Create_book_project.py'
    ...
    def test_real_directory_creation(self):
        ...
        # 模拟命令行参数
        with patch('sys.argv', ['create_book_project.py', test_project_path, 'test_book']):
            ...
            with patch('common.print_banner') as mock_banner:
                ...
                # 验证解压被调用
                expected_target = os.path.join(test_project_path, 'test_book')
                mock_zip.extractall.assert_called_once_with(expected_target)
    ...
    def test_main_script_name_extraction(self, mock_zipfile, mock_rmtree, mock_makedirs,
        ...
        with patch('os.path.basename') as mock_basename:
            mock_basename.return_value = 'create-book-project'
            ...
            self.assertIn('create-book-project', str(first_call))
